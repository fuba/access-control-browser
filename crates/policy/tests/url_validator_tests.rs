...
fn assert_blocked(url: &str, cfg: &acb_policy::CompiledPolicy, predicate: impl FnOnce(&BlockReason) -> bool) {
    match validate_url(url, cfg) {
        Ok(_) => panic!("expected block for {url}, got allow"),
        Err(reason) => assert!(predicate(&reason), "wrong block reason for {url}: {reason:?}"),
    }
}
...
fn fqdn_exact_rejects_subdomain() {
    let cfg = fixture();
    assert_blocked("https://api.github.com/users", &cfg, |r| matches!(r, BlockReason::NoMatchingRule));
}
...
fn fqdn_subdomain_wildcard_rejects_lookalikes() {
    ...
    assert_blocked("https://evilwikipedia.org/", &cfg, |r| matches!(r, BlockReason::NoMatchingRule));
    assert_blocked("https://wikipedia.org.attacker.example/", &cfg, |r| matches!(r, BlockReason::NoMatchingRule));
}

#[test]
fn regex_rule_matches_anchored() {
    let cfg = fixture();
    assert_allowed("https://docs.internal.example.com/api/v1", "internal-docs", &cfg);
    assert_allowed("https://docs.internal.example.com/guide", "internal-docs", &cfg);
}
...
fn ip_cidr_rejects_wrong_port() {
    let cfg = fixture();
    assert_blocked("http://10.1.2.3:8080/", &cfg, |r| matches!(r, BlockReason::NoMatchingRule));
}
...
fn ip_cidr_does_not_match_dns_name() {
    ...
    assert_blocked("http://internal.example/foo", &cfg, |r| matches!(r, BlockReason::NoMatchingRule));
}

#[test]
fn javascript_scheme_blocked() {
    let cfg = fixture();
    assert_blocked("javascript:alert(1)", &cfg, |r| matches!(r, BlockReason::SchemeBlocked(_)));
}

#[test]
fn data_url_blocked() {
    let cfg = fixture();
    assert_blocked("data:text/html,<script>x</script>", &cfg, |r| matches!(r, BlockReason::SchemeBlocked(_)));
}

#[test]
fn file_scheme_blocked() {
    let cfg = fixture();
    assert_blocked("file:///etc/passwd", &cfg, |r| matches!(r, BlockReason::SchemeBlocked(_)));
}

#[test]
fn chrome_scheme_blocked() {
    let cfg = fixture();
    assert_blocked("chrome://settings/", &cfg, |r| matches!(r, BlockReason::SchemeBlocked(_)));
}
...
fn ftp_scheme_blocked_even_if_not_in_deny_list() {
    ...
    let cfg = fixture();
    assert_blocked("ftp://example.com/", &cfg, |r| matches!(r, BlockReason::SchemeBlocked(_)));
}

#[test]
fn malformed_url() {
    let cfg = fixture();
    assert_blocked("not a url", &cfg, |r| matches!(r, BlockReason::MalformedUrl(_)));
    assert_blocked("://broken", &cfg, |r| matches!(r, BlockReason::MalformedUrl(_)));
}

#[test]
fn no_matching_rule() {
    let cfg = fixture();
    assert_blocked("https://random.example.org/", &cfg, |r| matches!(r, BlockReason::NoMatchingRule));
}
...
fn default_port_treated_as_known_default() {
    ...
    assert_blocked("http://10.1.2.3/", &cfg, |r| matches!(r, BlockReason::NoMatchingRule));
    assert_blocked("https://10.1.2.3/", &cfg, |r| matches!(r, BlockReason::NoMatchingRule));
}
